...
FLIST.DeleteFile          = function(dirId,dirPath)                                                 {
	if (confirm("Delete File,"+"\n"+"Are you sure?")) {
		if (FLIST.flgSample) {
			var apiDeleteFileResult = {status:1,response:""};
			var apiDeleteFileStr    = JSON.stringify(apiDeleteFileResult);
			FLIST.DeleteFileDone(apiDeleteFileStr,dirId,dirPath);
		} else {
			BF.HttpPostRequest(FLIST.apiDeleteFile,"path="+encodeURIComponent(dirPath)
				// delete success
				,function(strResponse) { FLIST.DeleteFileDone(strResponse,dirId,dirPath); }
			);
		}
	}
};
FLIST.DeleteFileDone      = function(strResponse,dirId,dirPath)                                     {
...
FLIST.MinifyJsFile        = function(dirId,dirPath)                                                 {
	if (confirm("Any previous minified versions of this JS"+"\n"+"will be overriden with the current version."+"\n\n"+"Continue generating?")) {
		...
		if (FLIST.flgSample) {
			var apiMinifyFileResult = {"status":1,"response":"","flgNew":true};
			var apiMinifyFileStr    = JSON.stringify(apiMinifyFileResult);
			FLIST.MinifyJsFileDone(apiMinifyFileStr,dirId,dirPath);
		} else {
			BF.HttpPostRequest(FLIST.apiMinifyJs,"path="+encodeURIComponent(dirPath)
				// minify success
				,function(strResponse) { FLIST.MinifyJsFileDone(strResponse,dirId,dirPath); }
			);
		}
	}
};
FLIST.MinifyJsFileDone    = function(strResponse,dirId,dirPath)                                     {
...
FLIST.MinifyCssFile       = function(dirId,dirPath)                                                 {
	if (confirm("Any previous minified versions of this CSS"+"\n"+"will be overriden with the current version."+"\n\n"+"Continue generating?")) {
		...
		if (FLIST.flgSample) {
			var apiMinifyFileResult = {"status":1,"response":"","flgNew":true};
			var apiMinifyFileStr    = JSON.stringify(apiMinifyFileResult);
			FLIST.MinifyCssFileDone(apiMinifyFileStr,dirId,dirPath);
		} else {
			BF.HttpPostRequest(FLIST.apiMinifyCss,"path="+encodeURIComponent(dirPath)
				// minify success
				,function(strResponse) { FLIST.MinifyCssFileDone(strResponse,dirId,dirPath); }
			);
		}
	}
};
FLIST.MinifyCssFileDone   = function(strResponse,dirId,dirPath)                                     {
	FLIST.dvLoader.style.display = "none";
	var objResponse = null;
	try { objResponse = JSON.parse(strResponse); } catch(e) { }
	if (objResponse && objResponse.status && objResponse.status == 1) {
...
FLIST.DeleteDir           = function(dirId,dirPath)                                                 {
	if (confirm("Delete whole folder!"+"\n"+"Are you sure?")) {
		if (FLIST.flgSample) {
			var apiDeleteDirResult = {status:1,response:""};
			var apiDeleteDirStr    = JSON.stringify(apiDeleteDirResult);
			FLIST.DeleteDirDone(apiDeleteDirStr,dirId,dirPath);
		} else {
			BF.HttpPostRequest(FLIST.apiRmDir,"path="+encodeURIComponent(dirPath)
				// delete success
				,function(strResponse) { FLIST.DeleteDirDone(strResponse,dirId,dirPath); }
			);
		}
	}
};
FLIST.DeleteDirDone       = function(strResponse,dirId,dirPath)                                     {
	var objResponse = null;
	try { objResponse = JSON.parse(strResponse); } catch(e) { }
	if (objResponse && objResponse.status && objResponse.status == 1) {
...
FLIST.CreateFileInDir     = function(dirId,dirPath)                                                 {
	...
	if (newFileName != null && newFileName != "") {
		...
		var newFileText = "";
		if (FLIST.flgSample) {
			var apiCreateFileResult = {status:1,response:""};
...
FLIST.CreateDirInDir      = function(dirId,dirPath)                                                 {
	...
	if (newFileName != null && newFileName != "") {
		...
		var newFileText = "";
		if (FLIST.flgSample) {
			var apiCreateDirResult = {status:1,response:""};
			var apiCreateDirStr    = JSON.stringify(apiCreateDirResult);
			FLIST.CreateDirInDirDone(apiCreateDirStr,dirId,dirPath,newFileName,newFilePath,newFileText);
		} else {
			BF.HttpPostRequest(FLIST.apiMkDir,"path="+encodeURIComponent(newFilePath)
				// creation success
				,function(strResponse) { FLIST.CreateDirInDirDone(strResponse,dirId,dirPath,newFileName,newFilePath,newFileText); }
			);
		}
	}
};
FLIST.CreateDirInDirDone  = function(strResponse,dirId,dirPath,newFileName,newFilePath,newFileText) {
	var objResponse = null;
	try { objResponse = JSON.parse(strResponse); } catch(e) { }
	if (objResponse && objResponse.status && objResponse.status == 1) {
		var dirList;
...
FLIST.UnzipFile           = function(dirId,dirPath)                                                 {
	if (confirm("The extracted data will override"+"\n"+"any data exists on the same location."+"\n\n"+"Continue unzipping?")) {
		...
		var newDirPath  = BF.ReplaceLast(dirPath, newFileName, "");
		if (FLIST.flgSample) {
			var apiUnzipResult = {status:1,response:""};
			var apiUnzipStr    = JSON.stringify(apiUnzipResult);
			FLIST.UnzipFileDone(apiUnzipStr,dirId,dirPath,newFileName,newDirPath);
		} else {
			BF.HttpPostRequest(FLIST.apiUnzip,"path="+encodeURIComponent(newDirPath) + "&file="+encodeURIComponent(newFileName)
				// unzip success
				,function(strResponse) { FLIST.UnzipFileDone(strResponse,dirId,dirPath,newFileName,newDirPath); }
			);
		}
	}
};
FLIST.UnzipFileDone       = function(strResponse,dirId,dirPath,newFileName,newDirPath)              {
	FLIST.dvLoader.style.display = "none";
	var objResponse = null;
	try { objResponse = JSON.parse(strResponse); } catch(e) { }
	if (objResponse && objResponse.status && objResponse.status == 1) {
...
FLIST.ZipDir              = function(dirId,dirPath)                                                 {
	if (confirm("Create a ZIP from this folder?")) {
		...
		if (FLIST.flgSample) {
			var apiZipResult = {status:1,response:""};
			var apiZipStr    = JSON.stringify(apiZipResult);
			FLIST.ZipDirDone(apiZipStr,dirId,dirPath,newDirName,newDirPath);
		} else {
			BF.HttpPostRequest(FLIST.apiZip,"path="+encodeURIComponent(newDirPath) + "&dirname="+encodeURIComponent(newDirName)
				...
				,function(strResponse) { FLIST.ZipDirDone(strResponse,dirId,dirPath,newDirName,newDirPath); }
			);
		}
	}
};
FLIST.ZipDirDone          = function(strResponse,dirId,dirPath,newDirName,newDirPath)               {
	FLIST.dvLoader.style.display = "none";
	var objResponse = null;
	try { objResponse = JSON.parse(strResponse); } catch(e) { }
	if (objResponse && objResponse.status && objResponse.status == 1) {
